...
class WorkstateBookRecord:
    def __init__(self, current_book_path, old_book_path,
        ...
        self.old_book_path: Path = self.book_path if old_book_path is None else Path(
            old_book_path)
        if not(description_stage is DescriptionStage):
            description_stage = DescriptionStage(int(description_stage))
    ...
    def __repr__(self):
        data = {
            ...
            'description_stage': str(self.description_stage.value)
        }
